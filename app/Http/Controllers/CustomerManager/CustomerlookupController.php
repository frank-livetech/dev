...
class CustomerlookupController extends Controller
{
            ...
            if($request->tag_emails != null && $request->tag_emails != '') {
                ...
                for( $i = 0; $i < sizeof($emails); $i++ ) {
                    ...
                    if($user) {
                        ...
                        $icon = 'at-sign';
                        $class = 'btn-success';
                        $desc = 'You were mentioned by '.Auth::user()->name . ' on Customer profile (<a href="'.url('/customer-profile').'/' .$note->customer_id.'">'.$note->customer->first_name .' '.$note->customer->last_name .'</a>) Note.';
                        ...
                        $mail->sendMail( '@'.auth()->user()->name .' has mentioned you on Customer Profile Note (' . $note->customer->first_name .' '.$note->customer->last_name .')' , $temp , 'system_mentioned@mylive-tech.com', $user->email , $user->name);
                    }
                }
            }
    ...
    public function search_customer(Request $request){
        ...
        try{
            ...
            $response = DB::select("SELECT cust.id,cust.avatar_url,cust.username,cust.first_name,cust.last_name,cust.email,cust.phone, comp.name, comp.id as company_id FROM customers cust Left JOIN companies comp ON cust.company_id = comp.id WHERE cust.username LIKE '%$id%' OR CONCAT(cust.first_name, ' ', cust.last_name) LIKE '%$id%' OR cust.email LIKE '%$id%' OR cust.phone LIKE '%$id%' OR comp.name LIKE '%$id%' ");
